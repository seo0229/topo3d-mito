...
class Trainer(TrainerBase):
    ...
    def __init__(self,
        ...
        if self.mode == 'train':
            ...
            # Always use pretrained model if specified
            pre_model = self.cfg.MODEL.PRE_MODEL
            if pre_model and (checkpoint is None or checkpoint != pre_model):
                print(f"Loading pretrained model weights from: {pre_model}")
                # Only load model weights, not optimizer state or iteration count
                pretrained_checkpoint = torch.load(pre_model, map_location=self.device)
                ...
                # Only load parameters that match in size
                for param_tensor in pretrained_dict:
                    if param_tensor in model_dict and model_dict[param_tensor].size() == pretrained_dict[param_tensor].size():
                        model_dict[param_tensor] = pretrained_dict[param_tensor]
                self.model.module.load_state_dict(model_dict)
                
                # Start from iteration specified in config, not from checkpoint
                self.start_iter = self.cfg.MODEL.PRE_MODEL_ITER
            else:
                # Normal checkpoint loading (model + optimizer + scheduler + iteration)
                self.start_iter = self.cfg.MODEL.PRE_MODEL_ITER  # Default starting point
    ...
    def train(self):
        ...
        for i in range(self.total_iter_nums):
            ...
            # ========== TOPOLOSS GRADIENT CHECK (ADD THIS) ==========
            # Only check on first iteration to avoid spam
            if i == 0 and self.is_main_process:
                print("\n" + "="*70)
                print("TOPOLOSS GRADIENT FLOW CHECK")
                print("="*70)
                
                topo_found = False
                for key, value in losses_vis.items():
                    if 'Topo' in key:
                        topo_found = True
                        print(f"\nLoss Name: {key}")
                        ...
                        print(f"  is_leaf: {value.is_leaf}")
                        
                        # Determine status
                        if isinstance(value, torch.Tensor) and value.requires_grad and value.grad_fn is not None:
                            print(f"  ✅ STATUS: GOOD - Gradients will flow!")
                        else:
                            print(f"  ❌ STATUS: BROKEN - Gradients will NOT flow!")
                            print(f"     -> Check topoloss.py for torch.tensor() bug!")
                
                if not topo_found:
                    print("\n⚠️  WARNING: No TopoLoss found in losses_vis!")
                    print("   Check your config to ensure TopoLoss is enabled.")
    ...
    def validate(self, iter_total):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(self.val_loader):
                ...
                with autocast(enabled=self.cfg.MODEL.MIXED_PRECESION):
                    ...
                    for key, value in losses_vis.items():
                        if key not in val_losses_individual:
                            val_losses_individual[key] = 0.0
                        val_losses_individual[key] += value.data if hasattr(value, 'data') else value
        ...
        if hasattr(self, 'monitor'):
            ...
            for key, value in val_losses_individual.items():
                self.monitor.logger.log_tb.add_scalar(
                    f'Validation_{key}', value, iter_total)
    ...
    def test(self):
        ...
        if self.output_dir is None:
            ...
            for k in range(len(result)):
                tiff_path = os.path.join(self.output_dir, f'prediction_{k:03d}.tiff')
                if result[k].ndim == 4:  # [C, D, H, W]
                    tifffile.imwrite(tiff_path, result[k])  # shape preserved
                    print(f'Saved multi-channel 3D TIFF: {tiff_path}')
            #     # Save each volume/channel separately if needed
            #     if result[k].ndim == 4:  # [C, D, H, W]
            #         for c in range(result[k].shape[0]):
            #             channel_path = os.path.join(self.output_dir, f'prediction_{k:03d}_channel{c}.tiff')
            #             tifffile.imwrite(channel_path, result[k][c])
                else:  # [D, H, W] or [H, W]
                    tifffile.imwrite(tiff_path, result[k])
