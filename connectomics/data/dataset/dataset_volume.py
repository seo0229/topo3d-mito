...
class VolumeDataset(torch.utils.data.Dataset):
    ...
    def __init__(self,
                 ...
                 # filtering empty layers
                 filter_empty_layers: bool = True,  # Parameters for activating filtering empty layers
                 min_fg_ratio: float = 0.001): # Threshold for considering a layer to have foreground
        ...
        # Filter out layers without foreground objects
        if self.mode == 'train' and self.label is not None:
            for vol_idx in range(len(self.volume)):
                # Identify valid layers
                valid_layers = np.zeros(self.label[vol_idx].shape[0], dtype=bool)
                
                # For each slice, check if it contains enough foreground
                for z in range(self.label[vol_idx].shape[0]):
                    fg_pixels = np.sum(self.label[vol_idx][z] != self.background)
                    total_pixels = self.label[vol_idx][z].size
                    
                    # Mark layer as valid if it contains sufficient foreground pixels
                    if fg_pixels / total_pixels >= 0.001:  # 0.1% foreground threshold
                        valid_layers[z] = True
                
                # Check if we found any valid layers
                if np.any(valid_layers) and not np.all(valid_layers):
                    print(f"Volume {vol_idx}: Filtering out {np.sum(~valid_layers)}/{len(valid_layers)} empty layers")
                    
                    # Keep only valid layers in both volume and label
                    self.volume[vol_idx] = self.volume[vol_idx][valid_layers]
                    self.label[vol_idx] = self.label[vol_idx][valid_layers]
                    
                    print(f"Volume {vol_idx} new shape: {self.volume[vol_idx].shape}")
                
                # Save the filtered volume and label as TIFF files
                save_dir = "filtered_volumes"
                os.makedirs(save_dir, exist_ok=True)
                
                tifffile.imwrite(os.path.join(save_dir, f"raw{vol_idx}.tiff"), self.volume[vol_idx].astype(np.uint16))
                tifffile.imwrite(os.path.join(save_dir, f"raw{vol_idx}gt.tiff"), self.label[vol_idx].astype(np.uint8))
